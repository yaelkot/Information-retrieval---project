...
class MultiFileWriter:
    ...
    def upload_to_gcp(self,path):
        ...
        blob = self.bucket.blob(f"{path}/{file_name}")
...
class MultiFileReader:
  ...
  def __init__(self):
    self._open_files = {}
    self.client=storage.Client()
    self.bucket = self.client.bucket("project_bucket_313294167")
...
class InvertedIndex:  
  ...
  @staticmethod
  def write_a_posting_list_gcp(b_w_pl, bucket_name,path):
    """
    like the above function, but with changes for gcp
    """
    posting_locs = defaultdict(list)
    ...
    with closing(MultiFileWriter(".", bucket_id, bucket_name)) as writer:
      for w, pl in list_w_pl:
        ...
        b = b''.join([(doc_id << 16 | (tf & TF_MASK)).to_bytes(TUPLE_SIZE, 'big')
                      for doc_id, tf in pl])
        # write to file(s)
        locs = writer.write(b,path)
        # save file locations to index
        posting_locs[w].extend(locs)
      writer.upload_to_gcp(path)
      InvertedIndex._upload_posting_locs(bucket_id, posting_locs, bucket_name,path)
    return bucket_id

  @staticmethod
  def _upload_posting_locs(bucket_id, posting_locs, bucket_name,path):
    """
    function that upload posting locs to the bucket
    """
    with open(f"{bucket_id}_posting_locs.pickle", "wb") as f:
      pickle.dump(posting_locs, f)
    client = storage.Client()
    bucket = client.bucket(bucket_name)
    blob_posting_locs = bucket.blob(f"{path}/{bucket_id}_posting_locs.pickle")
    blob_posting_locs.upload_from_filename(f"{bucket_id}_posting_locs.pickle")

  def get_posting_gen(self):
    """
